...
class ConfigManager:
    ...
    def validate_cluster_name(cluster: str) -> Tuple[bool, str]:
        ...
        if not ConfigManager._CLUSTER_NAME_RE.match(cluster):
            return (
                False,
                "Invalid cluster name: use lowercase a-z, 0-9, '-', '_' (1-63 chars)",
            )
        return True, "OK"
    ...
    def load_master_config(
    ...
    ) -> Tuple[MasterNodeConfig | None, str]:
        """Load master config.
        ...
        try:
            ...
            return MasterNodeConfig(
                hostname=hostname, ips=ips, kubernetes_version=version
            ), "OK"
        except Exception as e:
            return None, f"Failed to read config: {e}"

    @staticmethod
    def load_cluster_secrets(
        cluster: str | None = None,
    ) -> Tuple[ClusterSecrets | None, str]:
    ...
    def load_node_pools(
        cluster: str | None = None,
    ) -> Tuple[NodePoolsConfig | None, str]:
        ...
        try:
            ...
            for pool_data in raw_pools:
                ...
                azure_cfg = None
                aws_cfg = None
                aws_data = pool_data.get("aws") or None
                if isinstance(aws_data, dict):
                    asg_name = aws_data.get("asg_name")
                    region = aws_data.get("region", "us-east-1")
                    if asg_name:
                        from kitchen.config.models import NodePoolAwsConfig

                        aws_cfg = NodePoolAwsConfig(asg_name=asg_name, region=region)
                ...
                if provider_norm == "azure":
                    if azure_cfg is None:
                        return (
                            None,
                            f"Invalid node pool '{name}': provider is azure but 'azure' block is missing",
                        )
                    if (
                        not azure_cfg.vmss_name.strip()
                        or not azure_cfg.resource_group.strip()
                    ):
                        return (
                            None,
        ...
        try:
            ...
            def pool_to_dict(pool: NodePool) -> dict:
                ...
                if provider_norm == "azure":
                    ...
                    if (
                        not pool.azure.vmss_name.strip()
                        or not pool.azure.resource_group.strip()
                    ):
                        raise ValueError(
                ...
                return {
                    ...
                    "aws": (
                        {
                            "asg_name": pool.aws.asg_name,
                            "region": pool.aws.region,
                        }
                        if getattr(pool, "aws", None) is not None
                        else None
                    ),
                    "ssh": {
            ...
            new_pools_list = [
                pool_to_dict(pool) for pool in (pools_config.node_pools or [])
            ]
