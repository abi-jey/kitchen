...
class SSHSession:
    ...
    def __init__(
        ...
        self._passwordless_sudo = False  # Assume needs password until proven otherwise
    ...
    def _check_passwordless_sudo(self) -> None:
        ...
        """
        try:
            # sudo -n runs non-interactively, fails if password required
            stdin, stdout, stderr = self.client.exec_command("sudo -n true", timeout=5)
            exit_code = stdout.channel.recv_exit_status()
            self._passwordless_sudo = (exit_code == 0)
            if self.verbose and self._passwordless_sudo:
                typer.secho("  ✓ Passwordless sudo available", fg=typer.colors.GREEN)
        except Exception:
    ...
    def run(
        ...
        # If sudo needs a password, provide it. If passwordless sudo, just close stdin.
        # Generated by Copilot
        if use_sudo and not self._passwordless_sudo:
            if self._password:
                stdin.write(self._password + "\n")
                stdin.flush()
            else:
                # Need password but don't have it - prompt interactively
