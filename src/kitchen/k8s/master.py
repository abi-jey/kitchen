...
class MasterNode(BaseNode):
    ...
    def fix_apiserver_sans(self, tailscale_ip: str) -> bool:
        ...
        try:
            ...
            self.session.run(backup_command)
            ...
            self.session.run(delete_command)
            ...
            self.session.run(extract_command)
            ...
            self.session.run(kubeadm_command)
            ...
            self.session.run(apply_command)
            ...
            self.session.run(restart_command)
            ...
            try:
                ...
                if remote_config_path:
                    self.session.run(f"rm {remote_config_path}")

        except Exception as e:
            typer.secho(f"❌ An error occurred during SANs fix: {e}", fg=typer.colors.RED)
            return False

    def get_join_command(self, tailscale_ip: str) -> str:
        """
        ...
        raw_join_command = self.session.run(command)
    ...
    def _install_yq(self) -> None:
        ...
        # fmt: on
        self.session.run(install_command)
    ...
    def _verify_sans_update(self, tailscale_ip: str) -> None:
        ...
        output = self.session.run(verify_command)
        if tailscale_ip not in output:
